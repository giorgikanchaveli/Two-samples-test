...
function projectionGridMeasure(measure::Vector{Float64},a,b,nGrid)

    output = zeros(nGrid+1)

    nBottom = length(measure)
    for i=1:nBottom 
        output[projectionGrid(measure[i],a,b,nGrid)] += 1 / nBottom
    end
...
function dlip(measure1::Matrix{Float64}, measure2::Matrix{Float64}, a::Float64, b::Float64, nGrid::Int = 250,
...
#     return dlip(q_1.atoms, q_2.atoms, a, b, nGrid, nSteps, nRerun, tol)
# end
